...
export const getMetValues = asyncHandler(async (req, res) => {
...
export const saveWorkoutSession = asyncHandler(async (req, res) => {
  const { exerciseType, reps, durationSeconds, formScore } = req.body;

  if (!exerciseType || reps === undefined || durationSeconds === undefined) {
    res.status(400);
    throw new Error('exerciseType, reps, and durationSeconds are required');
  }

  const weightKg = req.user.weight;
  if (!weightKg) {
    res.status(400);
    throw new Error('User profile is missing weight; cannot estimate calories');
  }
